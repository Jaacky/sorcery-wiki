...
**Important Note:** - you must make the activity logging attributes you use accessible by using "attr_accessible" or by not naming them in "attr_protected", otherwise the writing with update_attributes! inside the gem will fail! Be sure to use attr_* only after the call to 'authenticates_with_sorcery!'.
